...
export const FunctionaryImport = () => {
    const [file, setFile] = useState<File | null>(null);
    const { importFunctionary, queryFunctionary } = useFunctionary();
    ...
    const handleImport = async () => {
        ...
        await importFunctionary.mutateAsync(file);
    ...
    return (
        <div>
            ...
            />
            <button type="button" className="bg-audittpurple text-white px-4 py-2 rounded cursor-pointer" onClick={handleImport}>{importFunctionary.isPending ? 'Importando...' : 'Importar'}</button>
        </div>
    );
}