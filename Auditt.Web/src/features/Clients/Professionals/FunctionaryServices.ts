...
export const getFunctionary = async (): Promise<
	MsgResponse<FunctionaryModel[]>
> => {
...
export const createFunctionaryServices = async (
	...
	const url = "api/functionaries";
	const response = await ApiClient.post<MsgResponse<FunctionaryModel>>(
		url,
		model
	);
...
export const updateFunctionaryServices = async (
	...
	const response = await ApiClient.put<MsgResponse<FunctionaryModel>>(
		url,
		model
	);
...
export const importFunctionaryServices = async (
	file: File
): Promise<MsgResponse<FunctionaryModel[]>> => {
	const url = "api/functionaries/import";
	...
	if (response.status !== 200 && response.status !== 201) {
		return {
			isSuccess: false,
			message: "Error al importar los funcionarios",
			isFailure: true,
			error: {
				code: response.status.toString(),
				message: response.statusText,
