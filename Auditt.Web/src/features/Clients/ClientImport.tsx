...
export const ClientImport = () => {
    ...
    const { importClient, queryClients } = useClient();
    ...
    const handleImport = async () => {
        ...
        await importClient.mutateAsync(file);
    ...
    return (
        <div>
            ...
            />
            <button type="button" className="bg-audittpurple text-white px-4 py-2 rounded cursor-pointer" onClick={handleImport}>{importClient.isPending ? 'Importando...' : 'Importar'}</button>
        </div>
    );
}